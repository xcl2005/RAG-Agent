...
第一次学习建议从 [无 Key / 无 Docker 的隔离评测实验室](evaluation-lab.md) 开始：公开虚构语料、
双配置对照、逐题结果与可复现记录。本文下面的 `eval_retrieval.py` 则针对已入库的真实 Hybrid 检索服务。
...
| `should_answer` | 是否应当回答，用于当前门控的误拒答 / 错误放行指标；不代表已验证生成正确性 |
