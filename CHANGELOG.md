...
- 单文档教学手册：先讲解再示例/跟做，覆盖招聘差距、项目原理、面试和跨电脑开发交接；旧题单改为可选复习。

- 上下文整理节点：同语义范围去重、跨来源覆盖、精确字符预算、锚点回退与节点统计。
- 8 份虚构语料 / 32 题的隔离离线评测实验室，提供基线与术语扩展的逐题对照及可复现信息。
- 离线面试练习 CLI、14 天学习路线、真实贡献写法和 2026-09 官方招聘样本映射。
...
- 修复上下文分隔符预算遗漏、被裁剪文本仍展示完整 quote、中间大块阻止后续小块进入的问题。
- 同步架构文档中过时的门控、文档删除和评测描述，删除未验证的“独立原创”默认简历模板。
